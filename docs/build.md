...
The graph is rebuilt, never updated. Curation happens in the relational database; a run exports it in full and builds a fresh graph. See [`schema.md`](schema.md) for what is written.
...
Nothing pre-exists in a fresh graph, so `MERGE` buys nothing and costs a lookup per row. Deduplicate in Python — the export is a full snapshot, so every distinct protein, publication, method and peptide is known before the first write — then write in batches.
...
`Interaction.n_descriptions` / `n_publications` / `n_methods` / `n_peptides` are computed in one pass at the end of the build, not maintained.
